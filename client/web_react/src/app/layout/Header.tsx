  ...
  return [
    {
      ...
      onClick: () => {
        agent.Account.setCulture("en-US");
      },
    },
    {
      label: "Srpski",
      icon: (
        <Flag code="srb" style={{ width: 24, height: 16, marginRight: 8 }} />
      ),
      onClick: () => {
        agent.Account.setCulture("sr");
      },
    },
  ];
};
...
const createConfigurationOptions = (): MenuOption[] => {
  return [
    ...
    {
      label: "Api Localization update",
      icon: <TranslateIcon />,
      route: "/apiLocalization",
    },
    {
      label: "Api services management",
...
export default function Header({ darkMode, handleThemeChange }: Props) {
  ...
  const getUserMenu = () => {
    if (!user)
      return (
        <DropdownMenu
          customAvatar={<AccountCircleIcon fontSize="large" />}
    ...
    <AppBar position="static">
      ...
      >
        ...
        <Box
          ...
          justifyContent="space-between"
          alignItems="center"
          sx={{ minHeight: 32, paddingY: 0.5 }}
        >
          <Box display="flex" alignItems="center">
            <DropdownMenu
              customAvatar={<LanguageIcon fontSize="medium" />}
              tooltip="Change language"
              options={createCultureOptions()}
            />
          </Box>
          ...
          <Box display="flex" alignItems="center">
            {user && ( //(user.roles?.includes('Admin') || user.roles?.includes('SuperAdmin')) &&
              <DropdownMenu
                customAvatar={<WarehouseIcon fontSize="large" />}
                tooltip="Warehouse"
            ...
            {user && ( //(user.roles?.includes('Admin') || user.roles?.includes('SuperAdmin')) &&
              <DropdownMenu
                customAvatar={<GroupIcon fontSize="large" />}
            ...
            {user && ( //(user.roles?.includes('Admin') || user.roles?.includes('SuperAdmin')) &&
              <DropdownMenu
                customAvatar={<SettingsIcon fontSize="large" />}
            ...
            {user && ( //(user.roles?.includes('Admin') || user.roles?.includes('SuperAdmin')) &&
              <DropdownMenu
                customAvatar={<ArchiveIcon fontSize="large" />}
                tooltip="Logs"
                options={createLogsOptions()}
              />
            )}
          </Box>
        ...
        <Box display="flex" justifyContent="center" alignItems="center" gap={2}>
          <Tooltip title="Home">
            <Box
              display="flex"
              alignItems="center"
              sx={{ ...secondRowIconStyles, marginRight: 2, marginLeft: -7 }}
              onClick={() => navigate("/")}
            >
              <HomeIcon fontSize="large" />
              <Typography variant="h6" sx={{ ml: 1, mr: 1 }}>
                E_commerce_net_react
              </Typography>
            </Box>
          </Tooltip>
